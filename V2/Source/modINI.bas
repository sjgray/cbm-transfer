...
Sub LoadINI()
    ...
    If Exists(INIFile) = True Then
        FIO = FreeFile
        
        Open INIFile For Input As FIO
        ...
        With frmOptions
            ...
            AddPathFlag = INIbool("PathHistory"):       .cbPathHistory.value = B2V(AddPathFlag)
            ...
            For J = 0 To 4
                Tmp = INIStr("Upath" & Format(J))
                If Tmp <> "" Then
                    UPath(J) = Tmp
                    .lblUPath(J).Caption = Tmp
                End If
            Next J
            ...
            UseFirstDrive = INIbool("UseFirstDrive"):   .cbUseFirstDrive.value = B2V(UseFirstDrive)                     'new 2021-10-06
            ...
            NIBPrompt = INIbool("NibPrompt"):          .cbNibPrompt.value = B2V(NIBPrompt)
            ...
            CBMFontName = INIStr("CBMFontName")
            CBMFontSize = INIStr("CBMFontSize")
            
            '-- Utility Path options
                        
            If UPath(0) = "" Then UPath(0) = "C:\Program Files\opencbm":    .lblUPath(0) = UPath(0)        'Use default OpenCBM path
            If UPath(1) = "" Then UPath(1) = VicePath:                      .lblUPath(1) = UPath(1)        'Use VICE Path from old setting
...
Public Sub SaveINI()
    ...
    Close 1: Open INIFile For Output As #1
    ...
    With frmOptions
        ...
        For J = 0 To 4
            PutINIValue "UPath" & Format(J), UPath(J)
        Next J
        '---- General Options
        PutINIValue "Theme", Theme
        PutINIValue "DriveNum", .cboDriveNum.ListIndex + 8
        PutINIValue "UseFirstDrive", .cbUseFirstDrive.value
        ...
        PutINIbool "NibPrompt", NIBPrompt
        ...
        PutINIValue "CBMFontName", CBMFontName
        PutINIValue "CBMFontSize", CBMFontSize
...
Public Function INIStr(varname As String) As String
    ...
    P = P + Len(Tmp): P2 = InStr(P, INIBuf, Cr)
