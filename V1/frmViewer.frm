   ...
   Begin VB.Frame frFont 
      ...
      Begin VB.Frame frControls 
         ...
         Begin VB.ComboBox cboTheme 
            Height          =   315
            ItemData        =   "frmViewer.frx":2DAA
            Left            =   2700
            List            =   "frmViewer.frx":2DC9
            Style           =   2  'Dropdown List
            TabIndex        =   165
            Top             =   180
            Width           =   1245
...
Public Sub DrawChrSet()
    ...
    If FontOffset > 32767 Then FontOffset = 32767
...
Public Sub ShowChr()
    ...
    If picV.Height >= FontH * ChrZoom * 15 Then
        picChr.PaintPicture picV.Image, 0, 0, SelZoom * 8, SelZoom * FontH, X, Y, 8 * ChrZoom, FontH * ChrZoom 'Draw the Character using set on screen (not buffer)
...
Private Sub SaveFont(ByVal Mode As Integer)
    ...
    Else
        Print #FFIO, Mid(VBuf, FontOffset + ChrPos, ChrPosEnd - ChrPos + 1);  'Write RANGE
    End If
    Close FFIO
...
Private Sub ConvertFont(ByVal N As Integer)
    ...
    Select Case N
        ...
        Case 5: B = 16: H = 32: Pad = ""                        'Read 32 bytes, write 16                - 8x16 font
    End Select
...
Private Sub cmdReassemble_Click()
    ...
    If Exists(Source) Then
        frmMain.PubDoCommand "acme", Tmp, "Assembling...", False     'Use ACME to assemble the file
        ...
        If Exists(TEMPFILE1) Then
            'MsgBox "Errors assembling!"                                                 'Reassembly did not complete
            lblInfo.Caption = LastCMDError
        End If
    Else
...
Private Sub lblLineNum_Click()
    ...
    Tmp = InputBox("Jump to line#:", "Jump To", "")
    
    N = Val(Tmp)
    If N > 0 And N < lstML.ListCount Then
        lstML.ListIndex = N - 1
        lstML.Selected(N - 1) = True
    End If
...
'---- Prompt to Save ASM Ouput to file
...
'---- Save ASM Output to specified File
...
'---- Toggle Split ML View
Private Sub cmdMLSplit_Click()
    MLSplitFlag = Not MLSplitFlag
    If MLSplitFlag = True Then
...
Private Sub txtLA_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then KeyAscii = 0: ViewIt ViewMode, VFileName, VName, VExt 're-load the file
End Sub
